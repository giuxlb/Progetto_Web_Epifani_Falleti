...
public class CreateTrasferimento extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			trasferimentoValue = trasferimentoDao.createTrasferimento(DestUserID, DestContoID, UserID, ContoID, amount, causale);
		...
		case(0): ctx.setVariable("errorMsg", "The Conto ID you entered dodn't match with any User ID");
		path = "/StatoConto.html";
		templateEngine.process(path, ctx, response.getWriter());
		break;
		case(1): ctx.setVariable("errorMsg", "The amount you entered isn't available in your balance");
		path = "/StatoConto.html";
		templateEngine.process(path, ctx, response.getWriter());
		break;
		case(2):
			path = getServletContext().getContextPath() + "/ConfermaTrasferimento.html";
